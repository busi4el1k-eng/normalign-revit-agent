...
# Găsește compilatorul Inno Setup (ISCC.exe). Acoperă și instalarea per-user
# (winget: %LOCALAPPDATA%\Programs\Inno Setup 6) pe lângă cele din Program Files.
$iscc = @(
...
# Ultima variantă: ISCC.exe din PATH.
if (-not $iscc) {
...
if (-not $iscc) {
  throw "Inno Setup (ISCC.exe) nu a fost gasit. Instaleaza cu 'winget install JRSoftware.InnoSetup' sau de la https://jrsoftware.org/isdl.php"
}
